...
// Derived from the sim's OWN crown line, not written down: this printed "~138 tall" for two
// proportion passes after the body shrank, and a goal height is argued as a RATIO against it.
const PLAYER_H = (({ y }) => y - (headY({ y }) - C.HEAD_R))({ y: 0 });
...
for (const h of H) {
  ...
  console.log(`GOAL_H ${String(h).padStart(3)} (${(h / PLAYER_H).toFixed(2)}x the ${PLAYER_H}px player) → ${(goals/N).toFixed(1)} goals/match | legendary ${hard} : ${easy} very-easy (${draw} draws)`);
}
