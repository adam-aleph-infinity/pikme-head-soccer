...
export const SFX = {
  ...
  // ---- PICK-UPS ---------------------------------------------------------
  // A meteor's telegraph is a WARNING and is allowed to be alarming. A crate's telegraph is
  // an INVITATION, and it has to sound like one — if the two families of cue were the same
  // shape you would flinch at the thing you are supposed to run towards. So: meteors get
  // noise and rumble, pickups get clean pitched tones, always rising.

  // "Something has appeared." Two soft notes, deliberately quiet — it must not step on the
  // meteor whistle when both are on the pitch at once.
  puWarn() {
    blip({ freq: 784, type: 'triangle', peak: 0.20, dur: 0.12 });
    blip({ freq: 1175, type: 'triangle', peak: 0.16, dur: 0.16, t: 0.10 });
  },
  // "…and now you can have it." Brighter, and an octave above the warning, so the two read
  // as one gesture finishing.
  puLive() {
    blip({ freq: 1568, type: 'sine', peak: 0.30, dur: 0.20 });
    blip({ freq: 2349, type: 'sine', peak: 0.16, dur: 0.16, t: 0.05 });
  },
  // Nobody wanted it. A little sigh downwards, and no low end at all — it is not an impact.
  puGone() { sweep({ from: 900, to: 320, type: 'sine', peak: 0.14, dur: 0.22 }); },
  // Any effect running out. Same sigh, shorter, so a match with several of them going does
  // not turn into a chorus.
  puEnd()  { sweep({ from: 620, to: 260, type: 'square', peak: 0.14, dur: 0.14 }); },

  // One per item, so you know what you picked up without reading the banner. Each is built
  // out of the thing it does: inflating, magnetising, charging, ringing metal, a spring,
  // and ice.
  puGrow() {
    sweep({ from: 260, to: 880, type: 'sine', peak: 0.34, dur: 0.30 });
    sweep({ from: 260, to: 880, type: 'triangle', peak: 0.18, dur: 0.30, detune: 10 });
    thud({ freq: 180, peak: 0.5, decay: 0.2, t: 0.28 });
  },
  puMagnet() {
    sweep({ from: 900, to: 220, type: 'sawtooth', peak: 0.26, dur: 0.34 });
    blip({ freq: 110, type: 'square', peak: 0.30, dur: 0.42 });
    blip({ freq: 165, type: 'square', peak: 0.16, dur: 0.42, t: 0.02 });
  },
  puCharge() {
    sweep({ from: 220, to: 1600, type: 'sawtooth', peak: 0.34, dur: 0.26 });
    sweep({ from: 220, to: 1600, type: 'sawtooth', peak: 0.24, dur: 0.26, detune: 18 });
    thud({ freq: 2200, q: 5, peak: 0.4, decay: 0.08, t: 0.24 });
  },
  puShield() {
    [587, 880, 1175].forEach((f, i) => blip({ freq: f, type: 'triangle', peak: 0.30, dur: 0.36, t: i * 0.05 }));
    thud({ freq: 2600, q: 6, peak: 0.28, decay: 0.1 });
  },
  puSpring() {
    sweep({ from: 240, to: 1500, type: 'sine', peak: 0.32, dur: 0.11 });
    sweep({ from: 1500, to: 520, type: 'sine', peak: 0.26, dur: 0.16, t: 0.11 });
    sweep({ from: 520, to: 1100, type: 'sine', peak: 0.14, dur: 0.12, t: 0.27 });
  },
  puIce() {
    [2349, 1976, 1568, 1319].forEach((f, i) => blip({ freq: f, type: 'sine', peak: 0.24, dur: 0.24, t: i * 0.045 }));
    thud({ freq: 4200, q: 7, peak: 0.3, decay: 0.14 });
  },

  // The shield paying for a power shot: glass giving way, not metal ringing. It has to be
  // audibly DIFFERENT from `blocked`, because the whole point is that this block was free.
  puShieldBreak() {
    thud({ freq: 3200, q: 8, peak: 0.75, decay: 0.13 });
    [1976, 1568, 1175, 784].forEach((f, i) => blip({ freq: f, type: 'triangle', peak: 0.26, dur: 0.2, t: i * 0.035 }));
  },
};
